import code_analysis.util.ast_base as iAST
...
"""
    ...
    """
    actions : dict[type[iAST.ASTBase], set[callable]] = field(default_factory=lambda: defaultdict(set))

    actions : dict[str, set[callable]] = field(default_factory=lambda: defaultdict(set))

    current_path : list[iAST.ASTBase] = field(init=False, default_factory=list)
    ...
    def flatten_for_classes(self, *classes):
        ...
        if not classes and hasattr(iAST, '__all__'):
            classes = [getattr(iAST, x) for x in iAST.__all__
                       if issubclass(getattr(iAST, x), iAST.ASTBase)]
    ...
    def visit(self, node, *, skip_actions=False):
        """Visit a node."""
        assert isinstance(node, iAST.ASTBase)
    ...
    def visit_all(self, nodes:list[iAST.ASTBase]):
        for x in nodes:
            self.visit(x)
