...
from doot.mixins.action.zipper import ZipperMixin
import sh

from_key   = DootKey.make("from")
update_key = DootKey.make("update_")
target_key = DootKey.make("target")

basic_find = sh.find
corana_dir = ".corana"
corana_file = ".corana-meta.toml"
...
def readonly_raw(spec,state):
    ...
    not_meta   = lambda x: corana_dir not in x.parts
    not_toml   = lambda x: corana_file not in x.parts
